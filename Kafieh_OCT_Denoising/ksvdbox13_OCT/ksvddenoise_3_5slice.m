function [y,D,nz] = ksvddenoise_3_5slice(params,msgdelta)
...
if (p==1)
  ids{1} = reggrid(length(x)-blocksize+1, trainnum, 'eqdist');
    ...
    %----------------
    xt=x(:,:,1);
    b=[blocksize(1) blocksize(2)];
    [ids{1},ids{2}] = reggrid(size(xt)-b+1, trainnum);
    ids{3}=1;
    %----------------
...
b=[blocksize(1), blocksize(2), size(x,3)];
params.data = sampgrid(x,b,ids{:});
%-----------
...
elseif (p==3)
  [y,nz] = ompdenoise3_5slice(params,msgdelta);
else
