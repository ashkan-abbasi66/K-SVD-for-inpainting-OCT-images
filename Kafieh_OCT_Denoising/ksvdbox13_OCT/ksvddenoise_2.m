...
if (numel(blocksize)==1)
  blocksize = ones(1,p-1)*blocksize;
end
blocksize(p)=4;
...
%   [ids{:}] = reggrid(size(x)-blocksize+1, trainnum, 'eqdist');
    [ids{1}] = reggrid(size(x,1)-blocksize+1, trainnum, 'eqdist');
    [ids{2}] = reggrid(size(x,2)-blocksize+1, trainnum, 'eqdist');
    [ids{3}] = reggrid(size(x,3), trainnum, 'eqdist');
end
params.data = sampgrid_2(x,blocksize,ids{:});
